...
Interrupting the run keeps the corpus. Ctrl-C reaches your suite exactly as it would without the
recorder in the way — once, so the suite runs its own cleanup rather than reading a repeat as "quit
now" — and what the proxy observed up to that point is written before the recorder exits `130`.
Press it again to stop immediately, without waiting for the suite or the corpus. A `SIGTERM`, which
no terminal sends, is passed on to the suite and then handled the same way.
