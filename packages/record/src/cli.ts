 ...
 *
 * `SIGTERM` is never generated by a terminal, so it was aimed at this process and nothing else has
 * delivered it. For `SIGINT` the test is whether a terminal is attached at all, which is as close as
 * this gets: once a signal has arrived, nothing distinguishes a tty-generated one from a targeted
 * one. So it errs toward not sending a duplicate. The cost of being wrong that way is a `kill -INT`
 * aimed at a recorder that happens to be on a tty, where the suite then has to be stopped by the
 * second signal — against the cost of being wrong the other way, which is every ordinary Ctrl-C
 * cutting a suite's cleanup short.
 */
 ...
 * when the signal did not already reach it — see `alreadyDelivered`.
 *
  ...
  return new Promise((resolvePromise, reject) => {
    ...
    for (const signal of FORWARDED_SIGNALS) {
      const handler = (): void => {
        ...
        release();
        interrupted = signal;
        if (shouldForward(signal, terminalAttached())) child.kill(signal);
      };
