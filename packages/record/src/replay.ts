 ...
 *
 * **Which classes that evidence reaches.** The first eight shapes were all
 ...
 * document. Step 5d (issue #89) did the same for `not-in`, `array-contains-any`, `IS_NOT_NULL` and
 * `IS_NOT_NAN`, with the same result. The operators no shape carries are `<=`, `>=` and `IS_NAN`,
 * each the inclusive or positive sibling of one that was measured (`<` and `>`, and `IS_NULL`'s
 * equality class); they carry the limit on that kinship rather than on a reading of their own. It is
 * still one operand, one collection and one index set.
 *
